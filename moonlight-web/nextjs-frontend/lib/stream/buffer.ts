/**
 * ByteBuffer for binary data serialization
 * Ported from old frontend for input message encoding
 */
...
export class ByteBuffer {
    ...
    private bytesUsed(amount: number, reading: boolean) {
        ...
        if (reading && this.position > this.limit) {
            throw new Error('Failed to read over the limit');
        }
    }
    ...
    putUtf8Raw(text: string) {
        ...
        if (result.read !== text.length) {
            throw new Error('Failed to put utf8 text');
        }
    }
    ...
    getU16(): number {
        const view = new DataView(this.buffer.buffer);
        const byte = view.getUint16(this.position, this.littleEndian);
    ...
    getU32(): number {
        const view = new DataView(this.buffer.buffer);
        const byte = view.getUint32(this.position, this.littleEndian);
